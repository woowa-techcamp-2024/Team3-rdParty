...
public class Handler
    ...
    private void process(
            ExecutorService executorService, Random random, int offset, int limit, String uri)
                        ...
                        try {
                            ...
                            if (proceed) {
                                while (true) {
                                    ...
                                    HttpResponse<String> getSeats =
                                            getPerformanceSeats(uri, authorizationHeader);
                                    ...
                                    HttpResponse<String> selectSeatResponse =
                                            selectSeat(uri, seat, authorizationHeader);
    ...
    private String login(int userNumber, String uri) {
        ...
        String messageBody = writeValueAsString(new LoginRequest(email, password));

        HttpRequest apiLoginRequest =
                HttpRequest.newBuilder(URI.create(uri + "/api/login"))
    ...
    private boolean secondStory(String uri, int userNumber, String authorizationHeader) {
        ...
        while (true) {
            ...
            RemainingCountResponse remainingCountResponse =
                    readValue(response.body(), RemainingCountResponse.class);
    ...
    private void fifthStory(
            String uri, int userNumber, SeatElement seat, String authorizationHeader) {
        ...
        HttpRequest apiReservationTicket =
                HttpRequest.newBuilder(URI.create(uri + "/api/tickets"))
                        .header("Content-Type", "application/json")
    ...
    private HttpResponse<String> getPerformanceSeats(String uri, String authorizationHeader) {
        HttpRequest apiTicketingRequest =
                HttpRequest.newBuilder(
                                URI.create(uri + "/api/performances/" + performanceId + "/seats"))
                        .GET()
                        .header(AUTHORIZATION, authorizationHeader)
                        .header(PERFORMANCE_ID, String.valueOf(performanceId))
                        .build();
    ...
    private HttpResponse<String> getRemainingCount(String uri, String authorizationHeader) {
        HttpRequest apiRemainingCountRequest =
                HttpRequest.newBuilder(
                                URI.create(uri + "/api/performances/" + performanceId + "/wait"))
                        .header(AUTHORIZATION, authorizationHeader)
                        .header(PERFORMANCE_ID, String.valueOf(performanceId))
                        .GET()
                        .build();
    ...
    private void leaveWaiting(String uri, String authorizationHeader) {
        HttpRequest leaveWaitingRequest =
                HttpRequest.newBuilder(URI.create(uri + "/performances/" + performanceId + "/wait"))
                        .DELETE()
                        .header(AUTHORIZATION, authorizationHeader)
                        .header(PERFORMANCE_ID, String.valueOf(performanceId))
                        .build();
    ...
    private HttpResponse<String> selectSeat(
            String uri, SeatElement seatElement, String authorizationHeader) {
        SeatSelectionRequest request = new SeatSelectionRequest(seatElement.seatId());
        String value = writeValueAsString(request);
        HttpRequest apiSelectSeatRequest =
                HttpRequest.newBuilder(URI.create(uri + "/api/seats/select"))
                        .POST(BodyPublishers.ofString(value))
                        .header("Content-Type", "application/json")
                        .header(AUTHORIZATION, authorizationHeader)
                        .header(PERFORMANCE_ID, String.valueOf(performanceId))
